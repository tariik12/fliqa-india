

const OurAwards = () => {
    return (
        <div className="bg-[#DDF8F3] grid grid-cols-2 p-11 mt-14 mb-7 gap-12 items-center">
           <div className="bg-[#4E99A4] grid grid-cols-2 p-11 gap-6 ">
            <img className="" src="https://i.ibb.co/C9q3GQH/Rectangle-208.png" alt="" />
            <img className=" " src="https://i.ibb.co/G7LwyPV/Rectangle-209.png" alt="" />
           </div>
           <div className="grid grid-cols-2  ">
            <div className="w-2/3">
            <div className=" bg-base-100 mb-4 h-14 rounded-xl shadow-xl   grid grid-cols-2  items-center">
                
                <img className="h-14 ms-2 rounded-xl" src="https://i.ibb.co/VDRvxNB/image-149.png" alt="" />
                 <p className="text-right me-3">49.5 %</p>
             </div>
             <div className=" bg-base-100 mb-4 h-14 rounded-xl shadow-xl   grid grid-cols-2  items-center">
                
                   <img className="h-14 ms-2 rounded-xl" src="https://i.ibb.co/V3V6C0x/image-152.png" alt="" />
                    <p className="text-right me-3">49.5 %</p>
                </div>
                <div className=" bg-base-100  h-14 rounded-xl shadow-xl    grid grid-cols-2  items-center">
                
                <img className="h-14 ms-2 rounded-xl" src="https://i.ibb.co/qC2ZXrN/image-151.png" alt="" />
                 <p className="text-right me-3">49.5 %</p>
             </div>
            
            </div>
            <div className="w-2/3">
            <div className=" bg-base-100 mb-4 h-14 rounded-xl shadow-xl    grid grid-cols-2  items-center">
                
                <img className="h-14 ms-2 rounded-xl" src="https://i.ibb.co/CQ7YrpH/image-removebg-preview-2022-07-23-T185243-1.png" alt="" />
                 <p className="text-right me-3">49.5 %</p>
             </div>
            <div className=" bg-base-100  mb-4 h-14 rounded-xl shadow-xl   grid grid-cols-2  items-center">
                
                   <img className="h-14 ms-2 rounded-xl" src="https://i.ibb.co/SwW9X4P/image-removebg-preview-2022-07-23-T185023-1.png" alt="" />
                    <p className="text-right me-3">49.5 %</p>
                </div>
            
                <div className=" bg-base-100  h-14 rounded-xl shadow-xl  grid grid-cols-2  items-center">
                  <img  className="h-14 ms-2 rounded-xl" src="https://i.ibb.co/dBC00jC/image-154.png" alt="" />
                    <p className="text-right me-3">49.5 %</p>
                </div>
            </div>
           </div>
        </div>
    );
};

export default OurAwards;